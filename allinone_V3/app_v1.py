                ...
                for _, r in bt_edited_dir.iterrows():
                    ...
                    if bt_is_live:
                        ...
                        try:
                            ...
                            bt_symbol_data_map[sym] = df_full  # keep the chart inspector's source in sync too
                        except Exception:
...
with tab_mobile:
    ...
    if "mobile_cards" in st.session_state:
        ...
        st.markdown("##### ➕ Add a custom stock")
        st.caption("Not tied to the Aligned filter above -- track anything you want.")
        mob_custom_col1, mob_custom_col2 = st.columns([3, 1])
        with mob_custom_col1:
            mob_custom_input = st.text_input(
                "Symbol", key="mobile_custom_input", placeholder="RELIANCE, TCS",
                label_visibility="collapsed",
            )
        with mob_custom_col2:
            mob_custom_add = st.button("Add", use_container_width=True)

        if mob_custom_add:
            raw_syms = [s.strip().upper().removesuffix(".NS") for s in mob_custom_input.replace("\n", ",").split(",") if s.strip()]
            if not raw_syms:
                st.warning("Type a symbol first.")
            else:
                mob_custom_cards = st.session_state.get("mobile_custom_cards", [])
                already_have = {c["Symbol"] for c in mob_custom_cards} | {c["Symbol"] for c in mob_cards}
                added_custom = 0
                with st.spinner("Fetching..."):
                    try:
                        custom_nifty_df, _ = cached_nifty500_list()
                        custom_symbol_to_industry = dict(zip(custom_nifty_df["Symbol"], custom_nifty_df.get("Industry", pd.Series(dtype=str))))
                    except Exception:
                        custom_symbol_to_industry = {}
                    for sym in raw_syms:
                        if sym in already_have:
                            continue
                        try:
                            df = cached_fetch_single(sym, "5m", "5d")
                            if df is None or df.empty:
                                st.warning(f"No data for {sym} -- check the symbol.")
                                continue
                            df_ind = compute_all_indicators(df)
                            result = score_symbol(df_ind, active_indicators, weights)
                            sector_idx = index_for_industry(custom_symbol_to_industry.get(sym))
                            mob_custom_cards.append({
                                "Symbol": sym,
                                "Signal": result["signal_label"],
                                "Trade Score": result["trade_score"],
                                "Chg %": day_change_pct(df),
                                "Sector": index_display_name(sector_idx),
                                "LTP": result.get("close"),
                                "EntryTime": df.index[-1],
                            })
                            ...
                            st.warning(f"Couldn't fetch {sym}: {e}")
                st.session_state["mobile_custom_cards"] = mob_custom_cards
                if added_custom:
                    st.success(f"Added {added_custom} below -- tick it and hit Start Monitoring.")

        mob_custom_cards = st.session_state.get("mobile_custom_cards", [])
        for card in mob_custom_cards:
            sym = card["Symbol"]
            is_buy = card["Signal"] in ("Buy", "Strong Buy")
            with st.container(border=True):
                c1, c2 = st.columns([2, 1])
                with c1:
                    st.markdown(f"**{sym}** (custom)  {'🟢' if is_buy else '🔴' if not is_buy and card['Signal'] != 'Neutral' else '⚪'} {card['Signal']}")
                    st.caption(f"{card['Sector']} · Chg {card['Chg %']:+.2f}%" if card["Chg %"] is not None else card["Sector"])
                with c2:
                    st.metric("Score", card["Trade Score"], label_visibility="collapsed")
                st.caption(f"LTP ₹{card['LTP']}")
                st.checkbox("Add to Monitor", key=f"mobile_pick_{sym}")
        if mob_custom_cards and st.button("🗑 Clear custom stocks"):
            st.session_state["mobile_custom_cards"] = []
            st.rerun()

        mob_all_cards = mob_cards + mob_custom_cards
        if mob_all_cards and st.button("▶ Start Monitoring Selected", type="primary"):
            monitor_list = st.session_state.get("mobile_monitor_list", [])
            existing_symbols = {m["Symbol"] for m in monitor_list}
            added = 0
            for card in mob_all_cards:
                sym = card["Symbol"]
