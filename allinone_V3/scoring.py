...
def signal_extension(row):
    ...
    REVERTED (back to this dead-zone version) after live testing: a fully
    continuous ramp (no dead zone) combined with VWAP active turned out to
    fade WAY more setups than intended -- too few Short candidates were
    surviving both filters together. Back to the more conservative
    version that only kicks in past 1.5 ATR of stretch. If picked back up
    later, worth re-testing Extension and VWAP's combined effect
    specifically (not just each alone), since that combination was the
    actual problem, not necessarily either one individually.
    """
    close = row.get("Close", 0)
    ema20 = row.get("EMA20", close)
    atr = row.get("ATR", 0)
...
def score_symbol(df, active_indicators: dict, weights: dict = None):
    ...
    Thin wrapper around _score_dataframe() using the WHOLE df (i.e. "now").
    See score_trend() below for scoring earlier points in the same history.
    """
    return _score_dataframe(df, active_indicators, weights)


def score_trend(df, active_indicators: dict, weights: dict = None, lookback: int = 5):
    """
    Scores the last `lookback` candles (not just the current one), by
    re-running the exact same scoring logic against progressively less-
    truncated versions of df -- i.e. "what would the Trade Score/Signal
    have shown at each of the last few candles". Needs no new data: every
    indicator column is already computed across the full history by
    indicators.py, so this just re-runs the (cheap) combination step
    multiple times instead of fetching or recomputing anything.

    Added after noticing the tool only ever answers "what does this look
    like right now" -- this answers "how did it get here": a signal that's
    been building (Buy -> Buy -> Strong Buy) is a different situation than
    one that just flipped from Neutral in a single candle, even if both
    show "Strong Buy" right now.

    Returns a list of dicts (oldest -> newest, ending at "now"), each with
    time/trade_score/signal_label -- or an empty list if there isn't
    enough history behind the window (the OLDEST point in the window still
    needs its own minimum lookback to avoid a misleading/garbage score
    right at the edge of the fetched data).
    """
    min_rows_per_point = 5  # same floor score_symbol itself requires
    if df is None or len(df) < lookback + min_rows_per_point - 1:
        return []
    ...
    for i in range(n - lookback, n):
        df_upto = df.iloc[:i + 1]
        result = _score_dataframe(df_upto, active_indicators, weights)
        trend.append({
            "time": df_upto.index[-1],
...
_SIGNAL_ABBREV = {
    "Strong Buy": "SB", "Buy": "B", "Neutral": "N",
    "Sell": "S", "Strong Sell": "SS", "No Data": "?",
}


def _signal_bucket(label):
...
def trend_summary(trend: list):
    """
    Turns score_trend()'s output into the two display-friendly values:

      - sequence string, e.g. "N -> B -> B -> SB -> SB"
      - conviction: 0-100, how much the recent history actually AGREES
        with and SUPPORTS the CURRENT (most recent) signal. Same
        agreement-ratio x average-strength shape as the existing
        Confidence score in _score_dataframe() below -- just applied
        across the trend window instead of a single candle.

        High conviction = the last few candles have consistently been in
        the same direction as right now (a signal that's been building).
        Low conviction = mixed/oscillating history, even if the CURRENT
        reading looks strong on its own -- e.g. a Strong Buy that just
        flipped from Neutral/Sell a candle ago.

        If the current signal itself is Neutral, conviction is 0 -- there
        isn't a direction to have conviction about.

    Returns (sequence_str, conviction) -- ("-", None) if trend is empty.
    """
    if not trend:
        return "-", None

    sequence = " -> ".join(_SIGNAL_ABBREV.get(t["signal_label"], "?") for t in trend)

    current_bucket = _signal_bucket(trend[-1]["signal_label"])
    agreeing_strengths = []
    for t in trend:
        if _signal_bucket(t["signal_label"]) == current_bucket and current_bucket != "Neutral":
            agreeing_strengths.append(abs(t["trade_score"] - 50) / 50)

    agreement_ratio = len(agreeing_strengths) / len(trend)
    avg_strength = float(np.mean(agreeing_strengths)) if agreeing_strengths else 0.0
    conviction = round(100 * agreement_ratio * (0.4 + 0.6 * avg_strength), 1)
    conviction = float(np.clip(conviction, 0, 100))

    return sequence, conviction


def _score_dataframe(df, active_indicators: dict, weights: dict = None):
    """
    Core scoring logic, operating on a (possibly truncated) dataframe --
    "now" is just df.iloc[-1] of whatever's passed in. Shared by
    score_symbol() (full df = "now") and score_trend() (progressively
    truncated df = "N candles ago"), so there's exactly one place this
    logic lives rather than two copies that could drift apart.
    """
