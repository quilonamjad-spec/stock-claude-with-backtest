...
## Mobile: added custom stock tracking — same session
Small addition per user request: a way to track any stock on Mobile, not
just what the Aligned Buy/Sell scan surfaces. Row placed exactly where
asked -- between the scanned cards and the "▶ Start Monitoring Selected"
button.

- Text input (comma-separated, `.NS` suffix optional/stripped) + "Add"
  button. Fetches fresh 5m data, scores it through the SAME pipeline as
  the scan (`compute_all_indicators` -> `score_symbol`, using whatever
  `active_indicators`/`weights` are currently set) and looks up its
  sector the same way, so a custom card is indistinguishable in behavior
  from a scanned one once added -- just not required to be 🟢-Aligned to
  get in.
- Renders as its own card, reusing the exact same dict schema as scanned
  cards (`Symbol`, `Signal`, `Trade Score`, `Chg %`, `Sector`, `LTP`,
  `EntryTime`) -- so "Start Monitoring Selected" treats
  `mob_cards + mob_custom_cards` as one combined list with zero special-
  casing needed downstream.
- Dedup guards against adding a symbol already present in either the scan
  results OR the custom list already.
- Custom list persists across a fresh Scan (own session-state key,
  `mobile_custom_cards`, untouched by the Scan Now button) -- same
  "doesn't get wiped by re-scanning" precedent as the monitor list
  itself. A "🗑 Clear custom stocks" button resets it.

Tested the parsing/dedup logic in isolation (mixed-case + `.NS` input,
skipping a symbol already in scan results, skipping a symbol already
added as custom, empty/whitespace-only input) -- all correct.
