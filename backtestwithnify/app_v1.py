...
from market_context import day_change_pct, day_change_pct_asof
...
with tab_screener:
    ...
    if "screener_results" in st.session_state:
        ...
        if index_chg is not None:
            ...
            st.caption(
                "Info only, not a filter: compare this against each stock's own "
                "**Chg % (today)** column below and the Signal, the way you'd eyeball "
                "index alignment manually before trusting a signal."
            )
        else:
...
with tab_backtest:
        ...
        for sym, df in bt_data_map.items():
            try:
                ...
                bt_rows.append({
                    ...
                    "Chg % (as of cutoff)": day_change_pct_asof(df, bt_date, bt_time),
                    "Trade Score": result["trade_score"],
                    "Confidence": result["confidence"],
                    "Signal": result["signal_label"],
                    "RSI": result.get("rsi"),
    ...
    if "bt_result_df" in st.session_state:
        ...
        if bt_index_chg is not None:
            ...
            st.caption(
                "Info only, not a filter: compare this against each stock's own "
                "**Chg % (as of cutoff)** column below and the Signal - the same top-down "
                "alignment check you'd do manually (index up + stock up -> trust a Buy)."
            )
        else:
