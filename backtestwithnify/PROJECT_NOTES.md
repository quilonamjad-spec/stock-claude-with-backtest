...
## Related idea raised but NOT built yet: per-stock trend-vs-MA gate
A trader friend of the user's suggested a stricter, different filter: only
allow longs when price is above its own (likely daily) moving average,
only shorts when below — a hard gate, not just an info column, and based
on the *stock's own* MA rather than the index. Discussed but intentionally
not implemented — flagged here in case the user comes back to it. If
built, the natural home is probably another as-of-cutoff column in
`backtest.py`/Backtest tab (daily MA period TBD — 20/50/200 not yet
decided) so the user can test it against their own history before trusting
it, same as the index-alignment feature above.
