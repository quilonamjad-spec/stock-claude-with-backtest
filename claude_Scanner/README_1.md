...
Mapping comes from NSE's own sectoral constituent lists (Bank, Auto, IT,
Pharma, FMCG, Metal, Realty, Energy, Financial Services); a stock not found
in any of them falls back to NIFTY 50 as the comparison index. Index prices
come from Yahoo Finance sectoral index tickers (e.g. `^CNXAUTO`, `^NSEBANK`)
— note NIFTY Financial Services specifically uses a plain `NIFTY_FIN_SERVICE.NS`
ticker rather than the `^CNX`-style prefix the others use; `^CNXFIN` is a
